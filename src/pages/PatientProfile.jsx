    ...
    <DashboardLayout>
      <header className="dashboard-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
            ...
            <h1 style={{ fontSize: 'min(1.875rem, 6vw)' }}>{patient.nome}</h1>
          </div>
          <p className="auth-subtitle hide-mobile" style={{ textAlign: 'left', marginBottom: 0 }}>
        ...
        >
          <Save size={18} />
          <span className="hide-mobile">{saving ? 'Salvando...' : 'Salvar Alterações'}</span>
          <span className="show-mobile">Salvar</span>
        </button>
      </header>
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    {imcClass && <span className="imc-badge" style={{ backgroundColor: imcClass.color, fontSize: '0.7rem' }}>{imcClass.label}</span>}
                  </div>
                </div>
                <div className="form-group">
      ...
      <section className="profile-section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '10px' }}>
          <h2 className="profile-section-title" style={{ marginBottom: 0 }}>
            <TrendingUp size={24} />
            <span className="hide-mobile">Consultas e Evolução</span>
            <span className="show-mobile">Evolução</span>
          </h2>
          <button className="btn-primary" style={{ width: 'auto' }} onClick={() => setModalOpen(true)}>
            <Plus size={18} /> <span className="hide-mobile">Nova Consulta</span> <span className="show-mobile">Nova</span>
          </button>
        </div>
                ...
                <div key={c.id} className="consultation-card">
                  <div className="consultation-header">
                    <div>
                      ...
                      {c.proximo_retorno && (
                        <div className="next-return-badge" style={{ marginLeft: '12px', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                          <Calendar size={14} />
      ...
      <section className="profile-section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '10px' }}>
          <h2 className="profile-section-title" style={{ marginBottom: 0 }}>
            <Utensils size={24} />
            <span className="hide-mobile">Planos Alimentares</span>
            <span className="show-mobile">Dietas</span>
          </h2>
          {!generatingMealPlan && !viewingPlan && (
            ...
            >
              <Plus size={18} />
              <span className="hide-mobile">Gerar Plano Alimentar</span>
              <span className="show-mobile">Gerar</span>
            </button>
          )}
        </div>
        ...
        {generatingMealPlan ? (
          ...
          <div className="dashboard-card" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, fontSize: '1rem' }}>Plano Alimentar</h3>
              <button className="btn-secondary" onClick={() => setViewingPlan(null)} style={{ width: 'auto', padding: '8px 16px' }}>
                Voltar
              </button>
            </div>
            ...
            {mealPlans.length > 0 ? (
              mealPlans.map(plan => (
                <div key={plan.id} className="meal-plan-card" style={{ cursor: 'pointer' }} onClick={() => setViewingPlan(plan)}>
                  ...
                  <span className="meal-plan-title">Plano #{plan.id.substring(0, 5)}</span>
                </div>
              ))
            ) : (
